...
const scrollLeft = useRef(0);
  ...
  for (let i = 1; i < sortedTimes.length; i++) {
    ...
    if (currentMinutes === prevMinutes + slotMinutes) {
      ...
      const [eh, em] = currentEnd
        .split(':')
        .map(Number);

      const endMinutes =
        eh * 60 + em + slotMinutes;
  ...
  const totalPrice = sortedTimes.reduce(
    (sum, t) => sum + getSlotPrice(field, t),
    0
  );
  ...
  return {
    ...
    totalMinutes:
      sortedTimes.length * slotMinutes
  };
}, [selectedSlots, fields, selectedDate]);
