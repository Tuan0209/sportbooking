...
public class UserService {
    ...
    // Chỉ cho phép sửa name, email, phone — KHÔNG được đổi role/status/password
    ...
    // Admin có thể sửa tất cả: name, email, phone, role, status, password,
    // coinBalance
    ...
    public UserResponse adminUpdateUser(String id, AdminUpdateUserRequest request) {
        ...
        if (request.getCoinBalance() != null)
            user.setCoinBalance(request.getCoinBalance());

        // Chỉ hash + cập nhật password nếu admin truyền vào
    ...
    public UserResponse updateAvatar(String userId, MultipartFile file, String imageUrl) throws IOException {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_FOUND));
